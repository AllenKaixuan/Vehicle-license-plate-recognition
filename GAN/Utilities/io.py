...
class DataLoader:
    ...
    def processXY(self, path):
        ...
        x = tf.image.resize(y, [size[0] / self.scale, size[1] / self.scale])  # 图片缩放
    ...
    def load(self, fileList, batchSize=24):
        ...
        data = fileList.map(self.processXY,
                            num_parallel_calls=tf.data.experimental.AUTOTUNE)  # 创建新的数据集,数据加载与变换过程并行到多个CPU线程上
        # 重复载入，在训练中会有时间控制
